...
coolog.keychan['coolog-appender-logentries'] = {
  key: process.env.LOGENTRIES_APIKEY
};

var logger = coolog.logger('html-puzzle/grabber.js', 'root')
  , grabber = {}
  ;
...
function _getAbsoluteUrl(baseUrl, pathUrl, isCss) {
  ...
  if (!isCss) {
    ...
    if (filePath.pathname.indexOf("/") === 0) {
      // absolute url
      host = base.host;
      separator = '';
    } else {
    ...
    return base.protocol + "//" + host + "/" + filePath.pathname;
  }
}
        ...
        if (error || response.statusCode !== 200) {
          ...
          async.eachLimit(backgroundImageLinks, 5, function (item, imageEachCallback) {
            ...
            if (matches === null || matches[0] === undefined) {
              ...
              abosoluteImageUrl = _getAbsoluteUrl(abosoluteStyleUrl, matches[0].replace('url("', '').replace("url('", '').replace('url(', ''), true);
              request({url: abosoluteImageUrl, encoding: 'binary'}, function (error, response, body) {
                if (error || response.statusCode !== 200) {
                  logger.warn("cannot get image", abosoluteImageUrl, error);
                } else {
